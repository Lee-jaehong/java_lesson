...
//C35 클래스를 1)객체로 만들어서 2) static 인 것들 테스트 합니다.
// (참고: main 메소드를 갖는 클래스를 객체로 만들어서 사용하지 않는것이 일반적입니다.)
	public static void main(String[] args) {
		...
		//static 요소 테스트: static 변수 또는 메소드는 클래스이름으로 사용.
        ...
        // static이 저장되는 메모리 영역(메소드 영역)과 객체(힙 영역)가 저장되는 메모리 영역은 다릅니다.
...
//		C35_GlobalVar.TEST=100; // 오류 :final 은 값 변경 못합니다.
		...
		//참고 지역 변수의 기본 초기값 - 배열일때만 0, 0.0, 참조타입은 null, 나머지는오류
		System.out.println("test3.count = " + test3.count);
		System.out.println("test3.message = " + test3.message);
		System.out.println("C35_GlobalVar = " + C35_GlobalVar.point);
	}
